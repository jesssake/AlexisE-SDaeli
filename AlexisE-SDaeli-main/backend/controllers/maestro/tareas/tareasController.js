...
class TareasController {
    ...
    // =====================================================
    // 👨‍🎓 VISTA POR ESTUDIANTE (DIAPOSITIVA) - NUEVOS MÉTODOS
    // =====================================================
    
    // OBTENER ESTUDIANTES CON ENTREGAS (PARA VISTA POR ESTUDIANTE)
    async obtenerEstudiantesConEntregas(req, res) {
        try {
            const maestroId = req.user.id;
            console.log('📊 Obteniendo estudiantes con entregas para maestro:', maestroId);
            
            // Obtener todos los estudiantes que han hecho entregas de tareas del maestro
            const [estudiantes] = await db.query(`
                ...
                FROM usuarios u
                INNER JOIN entregas_tareas et ON u.id = et.estudiante_id
                INNER JOIN tareas t ON et.id_tarea = t.id_tarea
            ...
            // Para cada estudiante, obtener sus entregas
            const estudiantesConEntregas = [];
            ...
            for (const estudiante of estudiantes) {
                const [entregas] = await db.query(`
                    SELECT 
                        ...
                        et.fecha_entrega,
                        et.calificacion,
                        et.comentario_alumno,
                        et.comentario_docente,
                        et.estado,
                        CASE 
                            WHEN et.fecha_entrega > t.fecha_cierre THEN 1 
                            ELSE 0 
                        END AS es_tardia
                    FROM entregas_tareas et
                    INNER JOIN tareas t ON et.id_tarea = t.id_tarea
                    LEFT JOIN materias m ON t.id_materia = m.id_materia
                    WHERE et.estudiante_id = ? AND t.created_by = ?
                    ORDER BY t.fecha_cierre DESC
                `, [estudiante.estudiante_id, maestroId]);
                
                estudiantesConEntregas.push({
                    estudiante_id: estudiante.estudiante_id,
                    nombre: estudiante.nombre,
                    total_entregas: estudiante.total_entregas,
                    entregas: entregas.map(e => ({
                        id_entrega: e.id_entrega,
                        id_tarea: e.id_tarea,
                        titulo_tarea: e.titulo_tarea,
                        instrucciones: e.instrucciones,
                        nombre_materia: e.nombre_materia || 'Sin materia',
                        materia_color: e.materia_color || '#718096',
                        fecha_limite: e.fecha_limite,
                        fecha_entrega: e.fecha_entrega,
                        calificacion: e.calificacion,
                        comentario_alumno: e.comentario_alumno,
                        comentario_docente: e.comentario_docente,
            ...
            console.log(`✅ ${estudiantesConEntregas.length} estudiantes con entregas procesados`);
            
            res.json({
                success: true,
                estudiantes: estudiantesConEntregas,
                total: estudiantesConEntregas.length
            });
            ...
            res.status(500).json({
                success: false,
                message: 'Error al obtener estudiantes con entregas',
                error: error.message
            });
        }
    }
    ...
    async obtenerEntregasPorEstudiante(req, res) {
        try {
            const { id } = req.params;
            const maestroId = req.user.id;
            
            console.log(`📊 Obteniendo entregas del estudiante ID: ${id} para maestro: ${maestroId}`);
            ...
            console.log(`✅ ${entregas.length} entregas encontradas para estudiante ${id}`);
            ...
            console.error('❌ Error en obtenerEntregasPorEstudiante:', error);
            res.status(500).json({
                success: false,
    ...
    // =====================================================
    // 🔍 DIAGNÓSTICO - VERIFICAR DATOS DE ESTUDIANTES Y ENTREGAS
    // =====================================================
    async diagnosticarDatos(req, res) {
        try {
            const maestroId = req.user.id;
            ...
            const [estudiantes] = await db.query(`
                ...
                LIMIT 10
            `);
            resultados.estudiantes = estudiantes;
            
            // 2. Verificar tareas del maestro
            const [tareas] = await db.query(`
                SELECT id_tarea, titulo, created_by, id_materia 
                FROM tareas 
                WHERE created_by = ?
            `, [maestroId]);
            resultados.tareasMaestro = tareas;
            
            // 3. Verificar entregas totales
            const [entregasTotales] = await db.query(`
                SELECT COUNT(*) as total FROM entregas_tareas
            `);
            resultados.totalEntregasEnDB = entregasTotales[0].total;
            
            // 4. Verificar entregas con tareas del maestro
            const [entregasMaestro] = await db.query(`
                SELECT 
                    et.id_entrega, 
                    et.estudiante_id, 
                    et.id_tarea, 
                    u.nino_nombre as estudiante_nombre,
                    t.titulo as tarea_titulo,
                    t.created_by as tarea_creador
                FROM entregas_tareas et
                INNER JOIN tareas t ON et.id_tarea = t.id_tarea
                INNER JOIN usuarios u ON et.estudiante_id = u.id
                WHERE t.created_by = ?
                LIMIT 20
            `, [maestroId]);
            resultados.entregasDeTareasDelMaestro = entregasMaestro;
            
            // 5. Verificar todas las entregas (sin filtro de maestro)
            const [todasEntregas] = await db.query(`
                SELECT 
                    et.id_entrega, 
                    et.estudiante_id, 
                    et.id_tarea,
                ...
                LIMIT 20
            `);
            resultados.todasEntregas = todasEntregas;
            
            // 6. Estadísticas
            resultados.estadisticas = {
                total_estudiantes: estudiantes.length,
                total_tareas_maestro: tareas.length,
                total_entregas_en_db: resultados.totalEntregasEnDB,
                total_entregas_maestro: entregasMaestro.length,
                total_entregas_sin_tarea: todasEntregas.filter(e => !e.tarea_creador).length
            };
            
            console.log('✅ DIAGNÓSTICO COMPLETADO:', resultados.estadisticas);
            
            res.json({
                success: true,
                maestro_id: maestroId,
                resultados: resultados,
                mensaje: `Encontradas ${entregasMaestro.length} entregas de tareas del maestro ${maestroId}`
            });
...
module.exports = {
    ...
    TareasController: {
        ...
        // NUEVOS MÉTODOS PARA VISTA POR ESTUDIANTE
        obtenerEstudiantesConEntregas: TareasController.applyMiddleware(new TareasController().obtenerEstudiantesConEntregas),
        obtenerEntregasPorEstudiante: TareasController.applyMiddleware(new TareasController().obtenerEntregasPorEstudiante),
        // MÉTODO DE DIAGNÓSTICO
        diagnosticarDatos: TareasController.applyMiddleware(new TareasController().diagnosticarDatos)
    },
