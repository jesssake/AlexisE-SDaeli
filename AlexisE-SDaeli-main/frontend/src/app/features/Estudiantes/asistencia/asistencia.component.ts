// C:\Codigos\HTml\AlexisE-SDaeli-main\AlexisE-SDaeli-main\frontend\src\app\features\Estudiantes\asistencia\asistencia.component.ts
...
export class EstudianteAsistenciaComponent implements OnInit {
  ...
  private cargarDatosEstudiante(): void {
    try {
      ...
      if (!userData) {
        ...
        this.logger.log('⚠️ Datos encontrados en localStorage (fallback);');
      }
      
      if (userData) {
        const usuario = JSON.parse(userData);
        
        // Obtener ID del estudiante (priorizar nino_id si existe)
        this.estudianteId = usuario.nino_id || usuario.id || usuario.userId || 0;
        
        // Obtener nombre del estudiante
        if (usuario.nino_nombre && usuario.nino_nombre !== 'null') {
          this.estudianteNombre = usuario.nino_nombre;
        } else {
          this.estudianteNombre = usuario.nombre || 'Estudiante';
        }
        
        // Obtener nombre del tutor
        if (usuario.tutor_nombre) {
          this.tutorNombre = usuario.tutor_nombre;
        } else if (usuario.nombre && usuario.rol === 'tutor') {
          this.tutorNombre = usuario.nombre;
        } else {
          this.tutorNombre = 'Tutor';
        }
        
        this.logger.log('👤 Datos del estudiante cargados desde sessionStorage:', {
          id: this.estudianteId,
          nombre: this.estudianteNombre,
          tutor: this.tutorNombre,
          rol: usuario.rol,
          nino_id: usuario.nino_id,
          nino_nombre: usuario.nino_nombre
        });
        
        if (!this.estudianteId || this.estudianteId === 0) {
          this.logger.warn('⚠️ No se pudo obtener ID del estudiante, datos disponibles:', usuario);
          this.error = 'No se pudo identificar al estudiante. Por favor, cierra sesión y vuelve a entrar.';
        }
      } else {
        this.logger.error('❌ No hay datos de usuario en sessionStorage ni localStorage');
  ...
  cargarAsistencia(): void {
    if (!this.estudianteId || this.estudianteId === 0) {
      this.error = 'ID del estudiante no disponible. Por favor, cierra sesión y vuelve a entrar.';
      this.logger.error('❌ ID de estudiante inválido:', this.estudianteId);
      return;
    }
    ...
    const params = new HttpParams()
      ...
      .set('año', this.anioSeleccionado)
      .set('limite', this.limiteRegistros.toString());

    this.logger.log('🔄 Cargando asistencia para estudiante:', {
      id: this.estudianteId,
    ...
    this.http.get<any>(`${this.apiBaseUrl}/${this.estudianteId}`, { params }).subscribe({
      next: (response) => {
        ...
        if (response.success || response.ok) {
          ...
          this.logger.log('📊 Datos cargados:', {
          ...
          if (this.asistencias.length === 0) {
            this.mensajeExito = 'No hay registros de asistencia para el período seleccionado';
          } else {
      ...
      error: (err) => {
        this.logger.error('❌ Error cargando asistencia:', err);
        if (err.status === 0) {
    ...
    this.http.get<any>(url, { params }).subscribe({
      next: (response) => {
        if (response.success || response.ok) {
          ...
          this.logger.log('📈 Resumen mensual cargado:', this.resumenMensual.length);
        }
      },
      error: (err) => {
        this.logger.error('Error cargando resumen mensual:', err);
      }
    });
  }
  ...
  cargarAsistenciaHoy(): void {
    ...
    this.http.get<any>(`${this.apiBaseUrl}/${this.estudianteId}/hoy`).subscribe({
      ...
      error: (err) => {
        this.logger.error('Error verificando asistencia de hoy:', err);
        this.cargando = false;
      }
    });
  }
  ...
  probarConexion(): void {
    this.logger.log('🔍 Probando conexión con el backend...');
    this.http.get(`${this.apiBaseUrl}/test`).subscribe({
      next: (data) => {
        this.logger.log('✅ Backend responde:', data);
        this.mensajeExito = '✅ Conexión con el backend establecida correctamente';
      },
      error: (err) => {
        this.logger.error('❌ Backend no responde:', err);
  ...
  // ✅ NUEVO: Obtener iniciales del estudiante
  getInitials(): string {
    if (!this.estudianteNombre) return '??';
