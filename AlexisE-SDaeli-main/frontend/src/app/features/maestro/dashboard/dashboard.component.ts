...
export class DashboardComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  aplicarSeleccion(): void {
    ...
    lista.forEach(a => {
      this.http.post(`${this.apiAvisosBase}/avisos/${a.id}/toggle`, {}).subscribe({
        complete: () => {
          hechos++;
        ...
        error: (error) => {
          this.logger.error('Error al aplicar selección:', error);
          this.mostrarNotificacion(this.obtenerMensajeError(error), 'error');
        }
      });
    });
  }
  ...
  toggleAviso(aviso: Aviso): void {
    this.logger.log(`🔄 Alternando estado del aviso ID: ${aviso.id}`);
    
    this.http.post(`${this.apiAvisosBase}/avisos/${aviso.id}/toggle`, {}).subscribe({
      next: (r: any) => {
        if (this.ok(r)) {
          aviso.activo = !aviso.activo;
          this.mostrarNotificacion(
            `Aviso "${aviso.titulo}" ${aviso.activo ? 'activado' : 'ocultado'} correctamente`,
            'success'
          );
          this.cargarAvisos();
          this.cargarTodosLosAvisos();
        } else {
          this.mostrarNotificacion('No se pudo actualizar el estado del aviso', 'error');
        }
      },
      error: (error) => {
        this.logger.error('❌ Error al alternar aviso:', error);
        this.mostrarNotificacion(this.obtenerMensajeError(error), 'error');
      },
    });
  }
  ...
  toggleTodosLosAvisos(): void {
    ...
    lista.forEach(a => {
      this.http.post(`${this.apiAvisosBase}/avisos/${a.id}/toggle`, {}).subscribe({
        ...
        error: (error) => {
          this.logger.error('Error al alternar todos los avisos:', error);
          this.mostrarNotificacion(this.obtenerMensajeError(error), 'error');
        }
      });
    });
  ...
  // =========================================================
  // NOTIFICACIONES (LOG) - CORREGIDO
  // =========================================================
  mostrarNotificacion(msg: string, tipo: 'success' | 'error' | 'warning' = 'success'): void {
    // ✅ CORREGIDO: Quitado el punto y coma dentro de la cadena
    this.logger.log(`[${tipo.toUpperCase()}] ${msg}`);
  }
